...
void Gui::render(Engine* engine, TCOD_Console* dest, int x, int y)
{
    ...
    m_console.clear({' ', colors::black, colors::black});
    ...
    for (const Message& msg : m_log)
    {
        ...
        tcod::print(m_console, {bar_width + 2, msg_y}, msg.text, fg, std::nullopt);
...
{
    tcod::draw_rect(m_console, {x, y, width, 1}, 0, std::nullopt, bg);

    int bar_width = static_cast<int>(value / max_val * width);
    if (bar_width > 0)
    {
        tcod::draw_rect(m_console, {x, y, bar_width, 1}, 0, std::nullopt, fg);
    }

    tcod::print(m_console, {x + width / 2, y}, tcod::stringf("%s : %g/%g", name.c_str(), value, max_val), colors::white, std::nullopt, TCOD_CENTER, TCOD_BKGND_NONE);
}

void Gui::render_mouse_look(Engine* engine)
{
    const Mouse& mouse = engine->mouse();
